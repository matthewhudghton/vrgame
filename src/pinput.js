var Pinput = (function () {
  ...
  var isKeyClicked = function (
    key,
    currentKeyStateArray,
    previousKeyStateArray
  ) {
    return (
      isKeyDown(key, currentKeyStateArray) &&
      !isKeyDown(key, previousKeyStateArray)
    );
  };

  // same as *isKeyClicked* but with mouse buttons
  var isButtonClicked = function (
    key,
    currentButtonStateArray,
    previousButtonStateArray
  ) {
    return (
      isButtonDown(key, currentButtonStateArray) &&
      !isButtonDown(key, previousButtonStateArray)
    );
  };

  Pinput.prototype.isReleased = function (combo) {
    return (
      !checkCombo(combo, this.mouseStates, this.keyStates) &&
